...
CLEAN=rm -rf
# Simulator
SIMAVRDIR=~/usr/local/bin/simavr
SIMAVR=simavr
...
verifyFuses: 
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(HIGH):r:-:$(HEX) -U $(LOW):r:-:$(HEX) || echo "Add additional -v to the \"avrdude -c ...\" command above for more details on the error"

defaultFuses: 
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(HIGH):w:$(HDEFAULT):$(RAW) -U $(LOW):w:$(LDEFAULT):$(RAW) || echo "Add additional -v to the \"avrdude -c ...\" command above for more details on the error" 

fuses: 
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(HIGH):w:$(H120):$(RAW) -U $(LOW):w:$(L120):$(RAW) || echo "Add additional -v to the \"avrdude -c ...\" command above for more details on the error"

# Disable just the JTAG fuse
disableJTAG: 
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(HIGH):r:$(HTXT):$(HEX) || echo "Add additional -v to the \"avrdude -c ...\" command above for more details on the error"
...
<<<<<<< HEAD
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(MEMORY):w:$<m1284p
=======
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(MEMORY):w:$< || echo "Add additional -v to the \"avrdude -c ...\" command above for more details on the error"
>>>>>>> 0f8f1d86cbd64089541fae3ebc75198cad8e0994
